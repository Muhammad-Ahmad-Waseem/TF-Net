...
outp_path = '/mnt/Ahmad/WHU_Aerial_Data/Preprocessed'
partitions = os.listdir(base_path)
tiff_folder = 'geo_referenced_rasters'
masks_folder = 'masks'
edges_folder = 'edges'
...
for partition in partitions:
    ...
    # tiff_img_path = "/mnt/Ahmad/Tiff_files/WHU_{}.tif".format(partition)
    ...
    if not os.path.exists(os.path.join(out_dir, "new_boundaries")):
        os.makedirs(os.path.join(out_dir, "new_boundaries"))
    ...
    for file in pbar:
        ...
        if not empty_mask:
            ...
            np.save(os.path.join(out_dir, "new_boundaries", file_name + "_boundary"), boundary)
        else:
            num_blank += 1
            pbar.set_description("{} samples are blank in {} partition".format(num_blank, partition))
            empty_array = np.zeros((512, 512))
            # np.save(os.path.join(out_dir, "masks", file_name + "_mask"), empty_array)
            np.save(os.path.join(out_dir, "new_boundaries", file_name + "_boundary"), empty_array)
